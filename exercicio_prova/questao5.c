...
void calcularSomatorioLinhasColunas(float array[][N], float somatorioLinha[N], float somatorioColuna[N], int n)
{
    for(int i = 0; i < n; i++)
    {
        somatorioLinha[i] = 0;
        ...
        for(int j = 0; j < n; j++)
        {
            somatorioLinha[i] += array[i][j];
            somatorioColuna[j] += array[j][i];
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    float somatorioLinha[N];
    float somatorioColuna[N];

    calcularSomatorioLinhasColunas(array,somatorioLinha,somatorioColuna, N);
    ...
    //// calcular linhas
    float mediaLinhas = 0, medianaLinhas = 0, desvioPadraoLinhas = 0;

    for (int i = 0; i < N; i++) {
        mediaLinhas += somatorioLinha[i];
    }
    mediaLinhas /= N;

    bubble_sort(somatorioLinha, N);

    medianaLinhas = calcularMedianaOrdenado(somatorioLinha, N);

    desvioPadraoLinhas = calculaDesvioPadrao(somatorioLinha, mediaLinhas, N);

    //// calcular colunas
    float mediaColunas = 0, medianaColunas = 0, desvioPadraoColunas = 0;

    for (int i = 0; i < N; i++) {
        mediaColunas += somatorioColuna[i];
    }
    mediaColunas /= N;

    bubble_sort(somatorioColuna, N);

    medianaColunas = calcularMedianaOrdenado(somatorioColuna, N);

    desvioPadraoColunas = calculaDesvioPadrao(somatorioColuna, mediaColunas, N);

    printf("\nSomatorio das Linhas(ordenado):\n");
    for (int i = 0; i < N; i++) {
        printf("%d: %.2f\n", i, somatorioLinha[i]);
    }

    printf("\nSomatorio das Colunas(ordenado):\n");
    for (int i = 0; i < N; i++) {
        printf("%d: %.2f\n", i, somatorioColuna[i]);
    }
    printf("\nMedia das Somatorias das Linhas: %.2f\n", mediaLinhas);
    printf("Mediana das Somatorias das Linhas: %.2f\n", medianaLinhas);
    printf("Desvio Padrao das Somatorias das Linhas: %.2f\n", desvioPadraoLinhas);

    printf("\nMedia das Somatorias das Colunas: %.2f\n", mediaColunas);
    printf("Mediana das Somatorias das Colunas: %.2f\n", medianaColunas);
    printf("Desvio Padrao das Somatorias das Colunas: %.2f\n", desvioPadraoColunas);
